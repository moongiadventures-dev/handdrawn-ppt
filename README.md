...
인스타 캐러셀이면 PNG로 끝나지만, **발표장에서 열려면 진짜 파일**이 필요합니다.

```
이걸 pptx로 뽑아줘
```

`scripts/make_pptx.py`가 삽화 + 텍스트를 16:9 슬라이드로 조립합니다. 출처는 슬라이드 하단과 **발표자 노트에 자동으로** 들어갑니다.

```bash
python scripts/make_pptx.py deck.json -o 발표자료.pptx
```

`scripts/deck.example.json`을 복사해서 내용만 갈아끼우면 됩니다. 레이아웃 4종:
`title`(표지) · `content`(기본) · `full`(그림이 주인공) · `quote`(전환)

> **폰트 주의** — pptx는 폰트 파일을 품지 않고 **이름만** 저장합니다. 발표할 PC에 [Gaegu](https://fonts.google.com/specimen/Gaegu)가 없으면 다른 폰트로 대체돼 손그림 느낌이 사라집니다. 미리 설치하거나, `deck.json`의 `"font"`를 `"맑은 고딕"` 등으로 바꾸세요.

> **파일 크기** — 원본 2K 이미지를 그대로 넣으면 9장에 26MB가 나옵니다. 기본적으로 폭 1800px로 줄여 넣어 약 13MB가 됩니다. `--max-image-width`로 조절할 수 있습니다.
